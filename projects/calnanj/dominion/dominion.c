...
/*Refactor Code so Cards are implemented in their own functions*/
int cardAdventurer(int *drawntreasure, struct gameState *state, int *cardDrawn, int *currentPlayer, int *temphand, int *z){
	...
	//Introduced ERROR here... took out -1 after [*currentPLayer] 
...
int cardFeast(struct gameState *state, int *temphand, int *currentPlayer, int *choice1){
...
int cardRemodel(struct gameState *state, int *currentPlayer, int *choice1, int *choice2, int *handPos){
...
int cardBaron(struct gameState *state, int *currentPlayer, int *choice1){
...
int cardGreat_hall(struct gameState *state, int *currentPlayer, int *handPos){
  //+1 Card
  drawCard(*currentPlayer, state);
...
int cardMinion(struct gameState *state, int *currentPlayer, int *handPos, int *choice1, int *choice2){

  int choice1_minion = *choice2;
  int choice2_minion = *choice1;
  int i;
  int j;
...
int cardSteward(struct gameState *state, int *currentPlayer, int *handPos, int *choice1, int *choice2, int *choice3){
...
int cardTribute(struct gameState *state, int *currentPlayer, int* handPos, int *nextPlayer, int *tributeRevealedCards){
...
int cardAmbassador(struct gameState *state, int *currentPlayer, int *handPos, int *choice1, int *choice2){
  int j;
  int i;
...
int cardCutpurse(struct gameState *state, int *currentPlayer, int *handPos){
  int i;
  int j;
  int k;
...
int cardEmbargo(struct gameState *state, int *currentPlayer, int *handPos, int *choice1){
...
int cardOutpost(struct gameState *state, int *currentPlayer, int *handPos){
  //set outpost flag
  state->outpostPlayed++;
...
int cardSalvager(struct gameState *state, int *currentPlayer, int *handPos, int *choice1){
...
int cardTreasure_map(struct gameState *state, int *currentPlayer, int *handPos){
  int i;
  int index;
...
//End refactor of code to make card implementation into their own functions//



//Begin Refactor of switch statement - Call cards in appropriate place in cardEffect //
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus){
 ...
 //Call cards in cardEffect here//
  if (card == adventurer)
    return cardAdventurer(&drawntreasure, state, &cardDrawn, &currentPlayer, temphand, &z);
  else if ( card == feast) 
    ...
    return cardRemodel(state, &currentPlayer, &choice1, &choice2, &handPos);
  else if (card == smithy)
    return cardSmithy(state, &currentPlayer, &handPos);
  else if ( card == village)
    return cardVillage(state, &currentPlayer, &handPos);
  else if ( card == baron)
    return cardBaron(state, &currentPlayer, &choice1);
  else if ( card == great_hall)
    return cardGreat_hall(state, &currentPlayer, &handPos);
  else if ( card == minion)
    return cardMinion(state, &currentPlayer, &handPos, &choice1, &choice2);
  else if ( card == steward)
    return cardSteward(state, &currentPlayer, &handPos,  &choice1, &choice2, &choice3);
  else if (card == tribute)
    return cardTribute(state, &currentPlayer, &handPos, &nextPlayer, tributeRevealedCards); 
  else if (card == ambassador)
    return cardAmbassador(state, &currentPlayer, &handPos, &choice1, &choice2);
  else if (card == cutpurse)
    return cardCutpurse(state, &currentPlayer, &handPos);
  else if (card == embargo)
    return cardEmbargo(state, &currentPlayer, &handPos, &choice1);
  else if (card == outpost)
    return cardOutpost(state, &currentPlayer, &handPos);
  else if(card == salvager)
    return cardSalvager(state, &currentPlayer, &handPos, &choice1);
  else if (card == sea_hag)
    return cardSea_hag(state, &currentPlayer);
  else if (card == treasure_map)
    return cardTreasure_map(state, &currentPlayer, &handPos);
  else return -1;
  
  //End refactor of original switch statement//
